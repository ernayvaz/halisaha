...
export async function POST(req: NextRequest, context: { params: Promise<{ id: string }> }) {
  ...
  if (!userId) {
    // This is a guest participant
    let finalGuestName = guestName;
    
    // If no guestName provided, generate sequential name
    if (!finalGuestName) {
      const existingGuests = await prisma.participant.count({
        where: { eventId: id, isGuest: true }
      });
      finalGuestName = `Guest ${existingGuests + 1}`;
    }
